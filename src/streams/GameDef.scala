...
trait GameDef {
   ...
   * the game.
   */
  def startBlock: Block = Block(startPos,startPos)   //me
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def neighbors: List[(Block, Move)] = List((left, Left),(right,Right),(up, Up ),(down, Down))  //me

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter(x=> x._1.isLegal)                   //me
      

    /**
     * Returns `true` if the block is standing.
     */
    def isStanding: Boolean = (b1.row == b2.row && b1.col == b2.col)                              //me
      

    /**
     * Returns `true` if the block is entirely inside the terrain.
     */
    def isLegal: Boolean = ( terrain(b1) && terrain(b2) )                                         //me
  }
